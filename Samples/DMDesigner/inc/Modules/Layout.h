...
#define POSFLAG_DEFSIZE        '@'          // 在pos属性中定义窗口的size，只在在定义x2,y2时有效
	...
	/// <summary>
	///		锚点布局内置实现，属性：<see cref="DMAttr::PosLayoutAttr"/>
	/// </summary>
	class Layout:public IDMLayout
	{
		DMDECLARE_CLASS_NAME(Layout,"Layout",DMREG_Layout);
	public:
		Layout();
	...
	public:// 辅助
		...
		// 设计器增加
		CStringA GetPit(PIT pit);
		CStringA GetItem(POS_ITEM* pItem);
		CStringA GetPosString();// 根据4个pos生成string,如失败,返回空

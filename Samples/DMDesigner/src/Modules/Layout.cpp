...
namespace DM
{
	...
	DMCode Layout::SetOwner(LPVOID lpWnd)
	{
	...
	DMCode Layout::UpdateChildLayout()
	{
		DMCode iErr = DM_ECODE_FAIL;
		do 
		{
	...
	int Layout::ParseItemValue(const POS_ITEM &item,int nMin, int nMax,bool bX)
	{
		int nRet	=	0;
		int nWid	=	nMax-nMin;
	...
	bool Layout::Update4(LPRECT lpRcContainer,OUT CRect &rcWindow)
	{
		int nRet = 0;
	...
	bool Layout::Update2(LPRECT lpRcContainer,OUT CRect &rcWindow)
	{
		bool bRet = false;
		do 
		{
	...
	bool Layout::UpdateFull(LPRECT lpRcContainer,OUT CRect &rcWindow)
	{
		bool bRet = false;
		do 
		{
	...
	bool Layout::UpdateAuto(LPRECT lpRcContainer,OUT CRect &rcWindow)
	{
		bool bRet = false;
		do 
		{
	...
	bool Layout::ParseItemByRect(CRect& rect,POS_ITEM& PosLeft,POS_ITEM& PosTop,POS_ITEM& PosRight,POS_ITEM& PosBtm)
	{
		bool bRet = false;
		do 
		{
			...
			//top
			ParseItemPos(PosTop,rcContainer.top,rcContainer.bottom,false,rect.top);

			if (PIT_OFFSET == PosRight.pit)
			{
				PosRight.nPos = (float)(rect.Width());
			}
			else
			{
				ParseItemPos(PosRight,rcContainer.left,rcContainer.right,true,rect.right);
			}

			if (PIT_OFFSET == PosBtm.pit)
			{
				PosBtm.nPos = (float)(rect.Height());
			}
			else
			{
				ParseItemPos(PosBtm,rcContainer.top,rcContainer.bottom,false,rect.bottom);
			}
			bRet = true;
		} while (false);
		return bRet;
	...
	bool Layout::ParseItemPos(POS_ITEM &item,int nMin, int nMax,bool bX,int nRet)
	{
		int nWid = nMax-nMin;
		int nPos = 0;
		switch(item.pit)
		...
		case PIT_CENTER: 
			nPos = DMABS(nRet - (nWid/2 + nMin));
			item.bMinus = nRet > (nWid/2 + nMin) ? false: true;
			break;

		case PIT_PERCENT: 
			if (nRet>nMax) 
				nRet = nMax;
			nPos = (int)(nRet - nMin)*100/nWid;
			break;
				...
				if (pRefWnd) //需要确定参考窗口是否完成布局
				{
					if (bX)
					{
						...
						else
						{
							...
							item.bMinus = nRet > refPos ? false : true;
						}
					}
					else
					{

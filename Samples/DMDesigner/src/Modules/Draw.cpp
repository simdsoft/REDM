...
namespace DM
{
	...
	DMCode DM::DMDraw::DrawBackground(IDMCanvas* pCanvas,IDMRegion* lpRgn,DUIWindow* pWnd)
	{
	...
	DMCode DMDraw::DrawForeground(IDMCanvas* pCanvas,IDMRegion* lpRgn,DUIWindow* pWnd)
	{
		DMCode iErr = DM_ECODE_FAIL;
		do 
		{
	...
	DMCode DMDraw::Draw(IDMCanvas* pCanvas, IDMRegion* lpRgn,DUIWindow* pWnd)
	{
		DMCode iErr = DM_ECODE_FAIL;
		do 
		{
	...
	DMCode DMDraw::Render(DUIWindow* pWnd)
	{
		DMCode iErr = DM_ECODE_FAIL;
		do 
		{
			...
			if (m_bOnlyOneRectRepaint)// 只有一个无效矩形
			{
				...
				m_pMemCanvas->SelectObject(g_pDMApp->GetFont(L""));
					...
					||rcInvalid.Height()+100<m_rcCanvas.Height())						// 大画布,无效区很小（gdi内部会优化）,绘制的效率还是要低于无效区大小的画布上直接绘制
				{
						...
						)
					{
						for (int i=0;i<iNum;i++)
						{
							...
							pTempCanvas->SelectObject(g_pDMApp->GetFont(L""));
	...
	DMCode DMDraw::GetCanvas(IDMCanvas** ppCanvas)
	{
		DMCode iErr = DM_ECODE_FAIL;
		do 
		{
	...
	DMCode DMDraw::ResizeCanvas(SIZE size)
	{
		DMCode iErr = DM_ECODE_FAIL;
		do 
		{

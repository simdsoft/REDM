...
#define  EDIT_DELTA           2
	...
	DMCode DUISize::DV_CreateChildWnds(DMXmlNode &XmlNode)
	{
		...
		for (int i=0;i<2;i++)
		{
			g_pDMApp->CreateRegObj((void**)&m_pEdit[i], "dsizeedit",DMREG_Window);
			if (m_pEdit[i])
			{
	...
	//
	DMCode DUISize::OnAttrTextColor(LPCSTR pszValue, bool bLoadXml)
	{
		do 
		{
	...
	DMCode DUISize::OnAttrCuretClr(LPCSTR pszValue, bool bLoadXml)
	{
		do 
		{
	...
	DMCode DUISize::OnAttrCuretAnimateCount(LPCSTR pszValue, bool bLoadXml)
	{
		do 
		{

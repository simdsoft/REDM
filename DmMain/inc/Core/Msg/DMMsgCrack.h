...
//      guoyou		2015-1-14	1.0			  MSG_WM_PAINT 进行修改，避免OnPaint死锁（by JVY)
//-------------------------------------------------------
#pragma once

namespace DM
{
#define _WTYPES_NS DM
#ifndef GET_X_LPARAM
...
#define BEGIN_MSG_MAP(theClass) \
	...
	BOOL theClass::IsMsgHandled() const \
	{ \
...
#define CHAIN_MSG_STATIC_MAP(theChainClass,OtherFun)\
	if(theChainClass::OtherFun(hWnd, uMsg, wParam, lParam, lResult,dwMsgMapID))\
	{\
		return TRUE; \
	}

//-------------------------------------------------------
// 通用的msgMapID默认为0,你也可以通过这个宏来切换
...
//------------------------------------------------------
// 反射处理函数，直接调用了ReflectNotifications函数（在DMCWnd中定义）
...
// void OnPaint(CDCHandle dc) msdn:An application should not call BeginPaint except in response to a WM_PAINT message. Each call to BeginPaint must have a corresponding call to the EndPaint function. 

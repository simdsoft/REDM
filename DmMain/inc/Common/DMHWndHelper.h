...
{ 
	...
	{
	public:
		static const char* SIZE_initsize;                                    ///< 如果创建窗口时传入的size为0，则使用它创建窗口初始大小, 示例:initsize="800,572"
		static const char* SIZE_minsize;                                     ///< 最小大小, 示例:minsize="600,500"
		static const char* RECT_maxinset;                                    ///< 全屏时，周边预留大小,可设置负值 示例:maxinset="50,50,50,50"
		static const char* SIZE_maxsize;                                     ///< 最大大小，示例:maxsize="800,800"，-1表示不做限制
		static const char* STRING_title;                                     ///< 窗口标题，默认不会显示出来 示例:title="dmmagic"
		static const char* STRING_regtip;                                    ///< 使用的默认tip注册类名,为NULL使用默认 示例:regtip="dui_tip"
		static const char* STRING_regdraw;                                   ///< 使用的默认draw注册类名,为NULL使用默认 示例:regdraw="dui_draw"
		static const char* STRING_transid;                                   ///< 使用的transid,为NULL使用默认custom 示例:transid="custom"
		static const char* bool_bresize;                                     ///< 窗口可改变大小,示例:bresize="1"
		static const char* bool_btranslucent;                                ///< 窗口使用UpdateLayeredWindow更新,示例:btranslucent="1"
		static const char* BYTE_alpha;                                        ///< 窗口透明度,透明窗口有两种,详见http://hgy413.com/1865.html 示例:alpha="0xc0"
		static const char* INT_h;                                            ///< hsl变换中h,值必须为[-180,180],代表的是人眼所能感知的颜色范围,0为默认值，h="-10"
		static const char* INT_s;											  ///< hsl变换中s,值必须为[0,200],色彩的饱和度，它用0%至100%的值描述了相同色相、明度下色彩纯度的变化。数值越大，颜色中的灰色越少，颜色越鲜艳,示例:s="10"
		static const char* INT_l;										      ///< hsl变换中l,值必须为[0,200],指的是色彩的明度,100为默认值，数值越大，越接近于白色,示例:l="10"
	};
...
namespace DM
{
	...
	public:
		DM_BEGIN_ATTRIBUTES()
			...
			DM_WSTRING_ATTRIBUTE(DMAttr::DMHWndAttr::STRING_title,m_strTitle,DM_ECODE_OK)
			...
			DM_WSTRING_ATTRIBUTE(DMAttr::DMHWndAttr::STRING_transid,m_strTransId,DM_ECODE_OK)
			
		DM_END_ATTRIBUTES()

	public:
		DMCode OnAttributeMinSize(LPCSTR lpszValue, bool bLoadXml);
		DMCode OnAttributeMaxSize(LPCSTR lpszValue, bool bLoadXml);
		DMCode OnAttributeResize(LPCSTR lpszValue, bool bLoadXml);
		DMCode OnAttributeTranslucent(LPCSTR lpszValue, bool bLoadXml);
		DMCode OnAttributeAlpha(LPCSTR lpszValue, bool bLoadXml);
		DMCode OnAttributeH(LPCSTR lpszValue, bool bLoadXml);
		DMCode OnAttributeS(LPCSTR lpszValue, bool bLoadXml);
		DMCode OnAttributeL(LPCSTR lpszValue, bool bLoadXml);

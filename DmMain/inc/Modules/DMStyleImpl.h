...
namespace DMAttr
{
	...
	{
	public:
		static const char* STRING_id;											///< styleID,相当于窗口的name，唯一标识,示例:id="style"
		static const char* SKIN_skin;											///< 窗口的style背景图,示例:skin="checkbox"
		static const char* SKIN_ncskin;										///< 窗口的styleNC背景图,仅在设置了ncmargin时有效,示例:ncskin="dui_frame"
		static const char* RECT_ncmargin;                                      ///< 窗口的style非客户区四边距,示例:ncmargin="2,2,2,2"
		static const char* STRING_cursor;                                      ///< 窗口的style默认光标,示例:cursor="hand"
		static const char* COLOR_clrbg;                                        ///< 窗口的style默认背景色,在背景图为空时使用,示例:clrbg="pbgra(ff,00,ff,ff)"
		static const char* COLOR_clrbghover;                                   ///< 窗口的style默认hover背景色,在背景图为空时使用,示例:clrbghover="pbgra(ff,00,ff,ff)"
		static const char* COLOR_clrbgpush;                                    ///< 窗口的style默认push背景色,在背景图为空时使用,示例:clrbgpush="pbgra(ff,00,ff,ff)"
		static const char* COLOR_clrbgdisable;                                 ///< 窗口的style默认disable背景色,在背景图为空时使用,示例:clrbgdisable="pbgra(ff,00,ff,ff)"
		static const char* COLOR_clrbgnoactive;                                ///< 窗口的style默认noactive背景色,在背景图为空时使用,示例:clrbgnoactive="pbgra(ff,00,ff,ff)"
		static const char* COLOR_clrnc;                                        ///< 窗口的style默认NC背景色,在NC背景图为空时使用,示例:clrnc="pbgra(ff,00,ff,ff)"
		static const char* COLOR_clrnchover;                                   ///< 窗口的style默认NC hover背景色,在NC背景图为空时使用,示例:clrnchover="pbgra(ff,00,ff,ff)"
		static const char* COLOR_clrncpush;                                    ///< 窗口的style默认NC push背景色,在NC背景图为空时使用,示例:clrncpush="pbgra(ff,00,ff,ff)"
		static const char* COLOR_clrncdisable;                                 ///< 窗口的style默认NC disable背景色,在NC背景图为空时使用,示例:clrncdisable="pbgra(ff,00,ff,ff)"
		static const char* COLOR_clrncnoactive;                                ///< 窗口的style默认NC noactive背景色,在NC背景图为空时使用,示例:clrncnoactive="pbgra(ff,00,ff,ff)"
		
		static const char* BYTE_alpha;                                         ///< 窗口的style透明度,会在背景图或背景色的alpha值再次预乘,示例:alpha="0x80"
		static const char* COLOR_clrtext;                                      ///< 窗口的style默认normarl文本色,示例:clrtext="pbgra(ff,00,ff,ff)"
		static const char* COLOR_clrtexthover;                                 ///< 窗口的style默认hover文本色,示例:clrtexthover="pbgra(ff,00,ff,ff)"
		static const char* COLOR_clrtextpush;									///< 窗口的style默认push文本色,示例:clrtextpush="pbgra(ff,00,ff,ff)"
		static const char* COLOR_clrtextdisable;                               ///< 窗口的style默认disable文本色,示例:clrtextdisable="pbgra(ff,00,ff,ff)"
		static const char* COLOR_clrtextnoactive;                              ///< 窗口的style默认noactive文本色,示例:clrtextnoactive="pbgra(ff,00,ff,ff)"

		static const char* FONT_font;											///< 窗口的style默认normarl字体,示例:font="face:宋体,size:0,weight:400,charset:0,underline:1,italic:1,strike:1",face:、weight:后多位，其余:后限制1位,允许空格!内部做了空格去除操作
		static const char* FONT_fonthover;										///< 窗口的style默认hover字体,示例:fonthover="face:宋体,size:0,weight:400,charset:0,underline:1,italic:1,strike:1",face:、weight:后多位，其余:后限制1位,允许空格!内部做了空格去除操作
		static const char* FONT_fontpush;										///< 窗口的style默认push字体,示例:fontpush="face:宋体,size:0,weight:400,charset:0,underline:1,italic:1,strike:1",face:、weight:后多位，其余:后限制1位,允许空格!内部做了空格去除操作
		static const char* FONT_fontdisable;									///< 窗口的style默认disable字体,示例:fontdisable="face:宋体,size:0,weight:400,charset:0,underline:1,italic:1,strike:1",face:、weight:后多位，其余:后限制1位,允许空格!内部做了空格去除操作
		static const char* FONT_fontnoactive;									///< 窗口的style默认noactive字体,示例:fontnoactive="face:宋体,size:0,weight:400,charset:0,underline:1,italic:1,strike:1",face:、weight:后多位，其余:后限制1位,允许空格!内部做了空格去除操作
		
		static const char* INT_textmode;                                       ///< 窗口的style默认textalign,使用数字表示,示例textmode="0x100"
		static const char* OPTION_align;                                       ///< 窗口的style默认水平align,left对应Align_Left,center对应Align_Center,right对应Align_Right,示例align="left"
		static const char* OPTION_valign;                                      ///< 窗口的style默认竖直align,top对应VAlign_Top,middle对应VAlign_Middle,bottom对应VAlign_Bottom,示例valign="top"
		static const char* bool_bdot;									    	///< 窗口的style默认文本过长时使用...,示例bdot="1"
	};
...
namespace DM
{
	...
	public:
		...
		CStringA                                    m_strSkinName;  ///< SKIN NAME,用NAME的原因是，在style加载时，skin不一定加载了！
		CStringA									m_strNcSkinName;///< 非客户区SKIN NAME
		
	    CStringA                                    m_strCursor;    ///< 光标NAME
